...
class Classifier(nn.Module):
    def __init__(self, vocab_size: int, pad_id: int, embed_dim: int, num_classes: int, att_type: str = 'full', decoder_type: str = 'linear') -> None:
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim, padding_idx=pad_id)
        self.manifold = Lorentz(k=1.0)
        self.attention = HypformerAttention(manifold=self.manifold, input_dim=embed_dim, att_type=att_type, num_heads=1, use_weight=True, heads_concat=False)
